...
| hadoop_cluster_install.py | 根据指定的配置，一键安装 hadoop 集群                         | 需要基于```jdk_cluster_install.py```                         |
|   zk_cluster_install.py   | 根据指定的配置，一键安装 zookeeper 集群                      | 需要基于```jdk_cluster_install.py```                            |
|  jdk_cluster_install.py   | 根据指定的配置，一键在集群中安装 Java 环境                   | 需要基于```linux_cluster_init.py```                             |
